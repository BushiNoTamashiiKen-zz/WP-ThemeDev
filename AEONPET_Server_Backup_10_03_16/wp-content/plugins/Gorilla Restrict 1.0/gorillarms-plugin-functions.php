<?php
/**
 * Plugin name:  HidePostContent Plugin
...
// Add filters
add_filter('the_content', 'restrict_categories_register_user');
...
function plugin_option() {
	...
	add_settings_field(
	'post-page-id',
	'<label for="post-content-id">Posts to be restricted</label>',
	'post_page_field',
	'gr-restrict-content-user',
	'gr_settings_section'
	);
	...
	register_setting('gr_settings_group', 'gr_post-content-id');
}

function post_page_field() {
	echo "Enter the Post or Page ID to be restricted. <br/>";
	echo '<input style="width:300px; height:40px" type="text" id="post-content-id" name="gr_post-content-id" value"' . get_option('gr_post-content-id') . '"><br/>';
}

// Restrict content on front-end
function restrict_categories_register_user($content) {
	global $post;
	$post_database = get_option('gr_post-content-id');
	$post_database = explode(',', $post_database); // Set array to store post ids separated by commas.
	...
	foreach ($post_database as $posts) {
		$posts = trim($posts);
		if($posts == $post -> ID){ // Make sure the post ID the user enters matches the DB value
...
//Function to exclude category display
...
/*function restrict_categories($categories) {	
	...
	function ga_template_tag() {
			...
			if (current_user_can('administrator')) { // check for the current user type (Need to put this in a function)
